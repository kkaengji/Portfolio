...
export const projects: Project[] = [
  {
    ...
    overview:
      "티켓팅은 단기간 트래픽이 폭발하는 구조라 '순번 공정성'과 'UI 안정성'을 동시에 확보하는 게 핵심 설계 과제였습니다. 클라이언트가 순번을 직접 계산하면 조작 가능성이 생기므로, 순번 발급은 서버에서만 처리하고 클라이언트는 폴링으로 상태를 수신하는 단방향 구조로 분리했습니다.\n\n잔여석처럼 빠르게 변하는 데이터는 Context API로는 캐시 무효화 타이밍 제어가 어려워 TanStack Query로 관리했고, 좌석 선택 같은 단순 전역 상태는 Redux의 보일러플레이트 없이 Zustand로 처리했습니다.\n\n매크로 방지는 단순 CAPTCHA 대신, K-POP 이미지 기반 의미 검증이 가능한 VQA 방식을 선택해 봇 우회 난이도를 높였습니다.",
    role: "프론트엔드 개발\nNext.js 기반 웹 서비스 개발 · 티켓팅 및 대기열 UI 구현 · API 연동 · 상태 관리",
    period: "2026.02 ~ 진행 중",
    features: [
      "Queue 기반 대기열 설계 — 순번 발급을 서버에서 처리, 클라이언트는 폴링으로 상태 수신해 트래픽 집중 시 UI 안정성 확보",
      "TanStack Query 서버 상태 관리 — 잔여석·티켓 상태 등 빠르게 변하는 데이터에 캐시 무효화 전략을 명시적으로 적용",
      "좌석 선택 이중 검증 구조 — 낙관적 UI 업데이트로 UX 확보, 결제 직전 서버 재검증으로 동시 선택 충돌 최소화",
      "VQA 기반 매크로 방지 — CAPTCHA 대비 K-POP 컨텍스트에서 의미 기반 검증으로 봇 차단 정확도 향상",
      "멤버십 등급 기반 권한 제어 — 팬 활동 데이터 기준 차등 접근, 찐팬 중심의 공정한 구조 설계",
    ],
    trouble:
      "동시 좌석 선택 충돌 문제 → 서버에만 의존하면 응답 전까지 사용자가 같은 좌석을 중복 선택 가능. 클라이언트에서 선택 즉시 낙관적으로 disabled 처리해 UX를 확보하되, 결제 직전 서버 재검증을 추가해 실제 충돌은 서버에서 최종 차단하는 이중 구조로 해결",
    images: [
