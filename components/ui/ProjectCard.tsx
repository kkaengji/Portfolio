...
export default function ProjectCard({ project, onClick }: ProjectCardProps) {
  return (
    <div className="card" onClick={() => onClick(project)} role="button" tabIndex={0}
      ...
      <div className="card-tags">
        {project.tags.map(tag => (
          <span key={tag.name} className={"tag" + (tag.color ? " " + tag.color : "")}>
            {tag.name}
          </span>
        ))}
      </div>
      {(project.github || project.demo) && (
        <div className="card-links" onClick={e => e.stopPropagation()}>
          ...
          {project.demo && (
            <a href={project.demo} target="_blank" rel="noopener noreferrer" className="card-link accent">
              Demo ↗
            </a>
          )}
        </div>
